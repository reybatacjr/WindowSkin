...
namespace WindowSkin.Controls
{
    public class WindowSkinForm : Form, IWindowSkinControl
    {
        ...
        private Rectangle _userButtonBounds;
        ...
        protected override void OnResize(EventArgs e)
        {
            ...
            _userButtonBounds = new Rectangle((Width - SkinManager.FORM_PADDING / 2) - 4 * STATUS_BAR_BUTTON_WIDTH, 0, STATUS_BAR_BUTTON_WIDTH, STATUS_BAR_HEIGHT);
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            g.DrawIcon(this.Icon, 12, 8);

            //Draw border
            using (var borderPen = new Pen(SkinManager.GetDividersColor(), 1))
            {
                g.DrawLine(borderPen, new Point(0, 1), new Point(Width - 2, 1));
                g.DrawLine(borderPen, new Point(Width - 2, 1), new Point(Width - 2, Height - 2));
                g.DrawLine(borderPen, new Point(0, Height - 2), new Point(Width - 2, Height - 2));
                g.DrawLine(borderPen, new Point(0, 1), new Point(0, Height - 2));

                if (!_maximized)
                {
                    Region = Region.FromHrgn(CreateRoundRectRgn(0, 0, Width, Height, 15, 15));
                    g.DrawArc(borderPen, new Rectangle(0, 0, 15, 15), 190f, 40f);
                    g.DrawArc(borderPen, new Rectangle(Width - 17, 1, 15, 15), 280f, 70f);
                    g.DrawArc(borderPen, new Rectangle(Width - 17, Height - 17, 15, 15), 10f, 70f);
                    g.DrawArc(borderPen, new Rectangle(0, Height - 17, 15, 15), 100f, 70f);
                }
                else
                {
            ...
            bool userInitial = ShowUserInitial && ControlBox;
            ...
            if (_buttonState == ButtonState.UserOver && userInitial)
                g.FillRectangle(hoverBrush, _userButtonBounds);
            if (_buttonState == ButtonState.UserDown && userInitial)
                g.FillRectangle(downBrush, _userButtonBounds);
            ...
            using (var formButtonsPen = new Pen(SkinManager.ACTION_BAR_TEXT_SECONDARY, 1))
            {
                int x = showMax && showMin ? _userButtonBounds.X : showMax ? _minButtonBounds.X : _maxButtonBounds.X;
                int y = showMax && showMin ? _userButtonBounds.Y : showMax ? _minButtonBounds.Y : _maxButtonBounds.Y;
                if (userInitial)
                {
                    g.FillEllipse(SkinManager.ColorScheme.UserBrush,
                                x + (int)(_userButtonBounds.Width * 0.12),
                                y + (int)(_userButtonBounds.Height * 0.18),
                                (float)_userButtonBounds.Width * 0.66f,
                                (float)_userButtonBounds.Height * 0.66f);
                    g.DrawString(UserInitial, SkinManager.CenturyGothicB_10, SkinManager.ACTION_BAR_TEXT_SECONDARY_BRUSH, new Rectangle(
                                    x + (int)(_userButtonBounds.Width * 0.16),
                                    y + (int)(_userButtonBounds.Height * 0.24),
                                    (int)(_userButtonBounds.Width * 0.66),
                                    (int)(_userButtonBounds.Height * 0.66)));
                }

                // Minimize button.
                if (showMin)
                {
                    x = showMax ? _minButtonBounds.X : _maxButtonBounds.X;
                    y = showMax ? _minButtonBounds.Y : _maxButtonBounds.Y;

                    g.DrawLine(
                        formButtonsPen,
                        x + (int)(_minButtonBounds.Width * 0.33),
                        y + (int)(_minButtonBounds.Height * 0.50),
            ...
            //Form title
            g.DrawString(Text, SkinManager.CenturyGothicB_12, SkinManager.ColorScheme.TextBrush, new Rectangle(SkinManager.FORM_PADDING, STATUS_BAR_HEIGHT, Width, ACTION_BAR_HEIGHT), new StringFormat { LineAlignment = StringAlignment.Center });
        }
        #endregion
        ...
        private void UpdateButtons(MouseEventArgs e, bool up = false)
        {
            ...
            {
                if(((showUser && (!showMin && !showMax) ||
                    showUser && showMin && !showMax) && _maxButtonBounds.Contains(e.Location)) ||
                    showUser && showMin && showMax && _userButtonBounds.Contains(e.Location))
                {
                    _buttonState = ButtonState.UserOver;
                }
